...
| **Gifting a catalogue product** (no recipient yet) | Product detail → "Gift Now" | Currently a placeholder, and *not* a wiring job: `Gift` requires `itemId` + `wishlistId` + a real `recipientId`, and `reserve()` starts from an item already on someone's wishlist. The built flow answers "someone wants X, I'll buy it for them"; the product page asks the opposite. Needs either (a) recipient-first — pick a saved person, save the product to **your own** list tagged for them, then out to the merchant (no `Gift` record; fits "Wishtick takes no payment") or (b) proposing an item onto a WishMate's list, which needs a new permission. **(a) chosen, not yet built** |
